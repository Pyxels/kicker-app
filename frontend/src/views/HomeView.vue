  ...
  <div class="w-full max-w-2xl mx-auto space-y-10">
    ...
    <section>
      ...
      >
        ...
        <div class="shrink-0">
          <PlusIcon
            class="w-10 h-10 text-white opacity-90 group-hover:opacity-100"
          />
        </div>
      </div>
    </section>
    ...
    <section>
      <h2 class="text-xl font-semibold mb-3 flex items-center gap-2">
        Ongoing Matches
        <span
          class="flex items-center gap-1 text-red-600 text-sm font-semibold"
        >
      ...
      <div v-if="liveMatches.length === 0" class="text-gray-500 italic">
        No active matches.
      </div>
...
async function updateMatches() {
  matches.value = (
    ((await safe(() =>
      pb
        .collection("match")
        .getFullList({ ...matchFetchOptions, sort: "-updated" }),
    )) as MatchDto[]) || []
  ).toSorted(
    (a, b) =>
